    ...
    <main className="app-shell">
      ...
      {view === "account" && currentUser && (
        <section className="product-page">
          ...
          <div className="operations-grid">
            <article className="operations-card"><p className="step-kicker">账户信息</p><h3>{currentUser.display_name}</h3><div className="account-details"><div><span>邮箱</span><strong>{currentUser.email}</strong></div><div><span>验证状态</span><strong>{currentUser.email_verified ? "已验证" : "待验证"}</strong></div><div><span>账户角色</span><strong>{currentUser.role === "admin" ? "管理员" : "Beta 用户"}</strong></div></div><button className="outline-button" onClick={() => void handleLogout()}>退出当前会话</button></article>
